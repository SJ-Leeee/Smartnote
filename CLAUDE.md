...
## 현재 진행 상태
- **완료**: Day 1 (Setup), Day 2 (Content Enhancement), Day 3 (Storage Layer)
- **진행 중**: Day 4 (User Feedback Loop - Rich diff 표시 및 UX 개선)
- **브랜치**: `feat/userfeedback-loop`

### 완료된 핵심 구현
- LangGraph 3-node 워크플로우: `enhance → feedback → save`
- Claude API Tool Calling 기반 콘텐츠 다듬기 (haiku-4-5 모델)
- 사용자 피드백 루프 (A/E/Q 인터랙션) - Day 2에 선구현
- Obsidian 파일 저장 (YAML frontmatter 자동 생성, 날짜 주입)
- Notion API 저장 (마크다운 → Block 변환)
- `--skip-notion` 플래그 동작 확인

### Day 4 남은 작업 (현재 브랜치)
- [ ] Rich 라이브러리 활용 원본 vs 보완본 diff 표시
- [ ] 사용자 입력 처리 UX 개선 (현재 기본 구현)
- [ ] E2E 테스트 (입력 → 저장 전체 흐름)
