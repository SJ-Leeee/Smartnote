...
## 현재 진행 상태
- **완료**: Day 1 (Setup), Day 2 (Content Enhancement), Day 3 (Storage Layer), Day 4 (User Feedback Loop), Day 5 (Category System), Day 6 (RAG Foundation)
- **다음**: Day 7 (MVP 통합 & 실전 테스트)
- **브랜치**: `feat/rag-foundation` (Day 6 완료)
...
- **ChromaDB note_id**: `uuid.uuid4()` → `obsidian_path` (중복 방지 upsert)
- **Obsidian related 링크**: `[[파일명|제목]]` alias 형식 (날짜 포함 파일명 + 사람이 읽기 좋은 제목)
- **기존 노트 bulk 임베딩**: 마이그레이션 작업 필요 (쓰다만 메모 처리 전략 포함) → 후순위 예정
