...
## 현재 진행 상태
- **완료**: Day 1~10 (Setup → Content Enhancement → Storage → Feedback Loop → Category System → RAG Foundation → MVP 통합 & 실전 테스트 → Bulk Migration → LangGraph 병렬화 → LLM-as-Judge)
- **다음**: Day 11 (Notion 메타데이터 전환 + Raw 원본 보존)
- **브랜치**: `feat/llm-as-judge`
...
- LangGraph fan-out/fan-in 병렬 아키텍처 구현 (classify ∥ find_related)
- `Annotated[dict, _merge_dict]` 리듀서로 병렬 노드 metadata 충돌 해결
- dispatch 노드 패턴으로 fan-out 구현 (조건부 엣지 리스트 불가 우회)
- 병렬 노드에서 UX 분리 → `node_user_input`으로 통합
- 타이밍 측정: classify ~4.5s (API 병목), find_related ~0.4s (로컬 DB)
- LLM-as-Judge 구현 (`core/judge.py`, `prompts/judge.txt`) - sonnet-4-6으로 haiku 출력 평가
- 루브릭 3개 기준: 원본보존/태그품질/가독성 (각 0~10, 총점 평균)
- 자동 재루프: 총점 < 8.0이면 issues를 feedback으로 주입 후 enhance 재시도 (최대 2회)
- `core/score_logger.py` - `logs/quality_scores.jsonl` phase별 점수 누적 (attempt_1, attempt_2)
- 사람 E 입력 시 `judge_retry_count` 리셋 → 새 방향에 대해 다시 품질 보장
...
- **fan-out 방식** (Day 9): 조건부 엣지 매핑 값에 리스트 불가 → dispatch 빈 노드 패턴 사용
- **병렬 노드 UX 금지** (Day 9): 빠른 노드가 먼저 prompt 띄우면 입력 충돌 → fan-in 후 순차 처리
- **note_id 전략** (Day 9 확정): 원본 소스 파일 절대 경로 (save/bulk 통일)
- **Judge 모델** (Day 10): sonnet-4-6으로 haiku 출력 평가. 같은 모델 자기평가는 신뢰도 낮음. 포폴 증거 수집 후 haiku로 교체 예정.
- **Judge 평가 기준** (Day 10): 원본보존 우선 (노트 목적은 "좋은 글"이 아닌 "정리된 나의 글") + 태그품질 (3~5개, 공백 없음, 상위 바운더리)
- **재시도 threshold** (Day 10): 총점 8.0. 실측 점수 분포 확인 후 결정 (낮으면 7.7, 대부분 8점대)
- **phase 구분 로그** (Day 10): attempt_1/attempt_2 → 자동루프 전후 품질 향상 포폴 증거용
